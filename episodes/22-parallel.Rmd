...
```bash
`r config$local$prompt` git clone git@github.com:NewcastleRSE-Training/HPC_Training_Example_Jobs.git
```

Compile the code.

```bash
`r config$local$prompt` cd HPC_Training_Example/c
`r config$local$prompt` module load GCC
`r config$local$prompt` ./compile.sh
```
...
```bash
`r config$local$prompt` mkdir primes
`r config$local$prompt` cd primes
`r config$local$prompt` cp ~/HPC_Training_Example_Jobs/c/multi ~/HPC_Training_Example_Jobs/c/single_gcc .
`r config$local$prompt` 
...
```bash
`r config$remote$prompt` ./single_gcc
```
...
```bash
`r config$remote$prompt` time ./single_gcc
```
...
```{r, child=paste(snippets, '/parallel/job_single.Rmd', sep=''), eval=TRUE}
```

Use the `r config$sched$name` status commands to check whether your job
is running and when it ends:

```bash
`r config$remote$prompt` `r config$sched$status` --me
```
...
```bash
`r config$remote$prompt` cat slurm-1177272.out
```
...
```{r, child=paste(snippets, '/parallel/job_multi.Rmd', sep=''), eval=TRUE}
```
