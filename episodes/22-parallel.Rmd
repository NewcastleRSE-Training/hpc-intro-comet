...
```bash
`r config$local$prompt` git clone git clone https://github.com/NewcastleRSE-Training/HPC_Training_Example_Jobs.git
```
...
`r config$local$prompt` module load GCC OpenMPI
...
```bash
`r config$remote$prompt` mkdir primes
`r config$remote$prompt` cd primes
`r config$remote$prompt` cp ~/HPC_Training_Example_Jobs/c/multi ~/HPC_Training_Example_Jobs/c/single_gcc .
...
snippets('parallel/job_single.Rmd')
```
...
```{r, echo=FALSE, results='asis'}
snippets('parallel/job_multi.Rmd')
```
